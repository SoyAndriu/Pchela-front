...
export default function SearchableClientSelect({ value, onSelect, darkMode }) {
  ...
  <label className="block text-sm mb-1">Cliente (buscar por DNI o email)</label>
      ...
      <ClienteFormModal
        ...
        darkMode={darkMode}
        onSaved={(c)=>{ setConsumidorFinal(false); onSelect && onSelect(c); setTerm(c.nombre_completo + ' - ' + c.email); }}
      />
    </div>
  );
}
